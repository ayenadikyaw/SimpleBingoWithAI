...
function horizontal(player) {
  ...
  for (let i = 0; i < tmpArr.length; i++) {
    ...
    for (let j = 0; j < tmpArr.length; j++) {
      if (tmpArr[i][j] == player) {  //r1[0],r2[0],r3[0],r4[0]...
        cnt++;
      }
    }
    if (cnt == 4) {
...
function computer() {
  ...
  for (let index = 0; index < tmpArr.length; index++) {
    if (horizontalProbability(tmpArr[index], first) == true) {
      flag = true;
    }
  }
  //check vertical win probability by player 1
  if (verticalProbability(tmpArr, first) == true) {
    flag = true;
    console.log("reachHere vertical" + ":" + flag);
  }

  //check diagonal win probability by player 1
  if (diagonalProbability(tmpArr, first) == true) {
    flag = true;
  }

  // if there is no win probability by player 1, check win probability by computer
  // if ther is win probability by computer, do it..otherwise set random location
  if (flag == false) {
 ...
 * @returns 
 */
function horizontalProbability(array, player) {
  let count = 0;
  let blank = 0;
  var isFree = false;
...
function verticalProbability(arr, player) {
  ...
  for (let i = 0; i < 4; i++) {
    var cnt = 0;
    for (let j = 0; j < 4; j++) {
      if (arr[j][i] == player) {    //r1-0,r2-0,r3-0,r4-0...
        cnt++;
        console.log(cnt);
      } else {
        row = j + 1;
